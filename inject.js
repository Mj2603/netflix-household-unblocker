/**
 * Netflix Household Unblocker - Content Script
 *
 * This script runs on Netflix pages (excluding /watch/) and intelligently
 * hides household verification modals using advanced DOM manipulation techniques.
 */

// Check if the script has already run in this context
if (window.hasRunNetflixUnblockerContentScript) {
    // console.log("NF Unblocker Content Script: Already running, exiting.");
} else {
    window.hasRunNetflixUnblockerContentScript = true;
    // console.log("NF Unblocker Content Script injected and running for the first time.");
    ...
    function hideModal(node) {
        ...
        if (node.matches && (node.matches(MODAL_SELECTOR) || node.querySelector(MODAL_DATA_UIA_SELECTOR))) {
            // console.log("NF Unblocker: Found modal, hiding.", node);
            ...
            modals.forEach(modal => {
                ...
                if (modal.querySelector(MODAL_DATA_UIA_SELECTOR)) {
                    // console.log("NF Unblocker: Found modal within added node, hiding.", modal);
    ...
    // console.log("NF Unblocker: MutationObserver is running.");
